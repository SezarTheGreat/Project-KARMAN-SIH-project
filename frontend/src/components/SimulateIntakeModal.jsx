...
export default function SimulateIntakeModal({ onClose, onIntakeCreated }) {
  ...
  return (
    ...
    >
      <div className="bg-[#241a2c] border border-[#79b473] rounded-xl max-w-lg w-full p-6 relative font-sans text-slate-100 shadow-2xl">
        
        <div className="flex items-center justify-between border-b border-[#3d2e49] pb-4 mb-4">
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-[#79b473]" />
            <h3 className="font-mono font-bold text-sm text-[#79b473]">SIMULATE WHATSAPP VOICE INTAKE</h3>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {presets.map((p, idx) => (
              <button 
                ...
                className="text-left bg-[#18121e] hover:bg-[#3d2e49] border border-[#3d2e49] p-2.5 rounded text-xs text-slate-200 transition"
              >
        ...
        <form onSubmit={handleSubmit} className="space-y-3 font-mono text-xs">
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-[#18121e] border border-[#3d2e49] rounded p-2 text-slate-100 font-sans focus:outline-none focus:border-[#79b473]"
              required
            ></textarea>
          </div>
